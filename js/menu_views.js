...
function renderHome(app) {
    ...
    return `<div class="bg-gray-200 h-[400px] relative overflow-hidden shadow-inner">
    ...
    </div><div class="container mx-auto px-6 py-16 text-center"><h2 class="text-xl font-bold text-[#1e3a2c] uppercase border-b-2 border-[#c5a059] inline-block pb-1 mb-8">${app.menuLabels[lang].home}</h2><div class="text-gray-700 italic mb-12">${data.intro ? data.intro[lang] : ''}</div><div class="flex justify-center items-center space-x-10 pt-8 border-t"><a href="#" class="text-2xl text-[#1e3a2c]"><i class="fa-brands fa-instagram"></i></a><a href="#" class="text-2xl text-[#1e3a2c]"><i class="fa-brands fa-linkedin"></i></a><a href="#" class="text-2xl text-[#1e3a2c]"><i class="fa-brands fa-youtube"></i></a><img src="https://espelhogrupo.cnpq.br/dgp/img/logo_cnpq.png" class="h-8 grayscale hover:grayscale-0"></div></div>`;
}

function renderDomains(app) {
    const doms = [{ id: 1, title: { pt: "Domínio 1 - Ciência, Tecnologia, Inovação e Sustentação em Defesa" } },{ id: 2, title: { pt: "Domínio 2 - Preparo e Emprego do Poder Militar" } },{ id: 3, title: { pt: "Domínio 3 - Análise e Prospecção do Ambiente Estratégico" } }];
    return `<div class="container mx-auto px-6 py-12">${doms.map(d => `<div class="mb-16"><h3 class="text-lg font-bold text-[#1e3a2c] uppercase border-l-8 border-[#c5a059] pl-4 mb-8">${d.title.pt}</h3><div class="grid grid-cols-2 md:grid-cols-4 gap-8">${(app.data.researchers || []).filter(r => r.domain_id == d.id).map(r => renderResearcherCard(r, app.lang)).join('')}</div></div>`).join('')}</div>`;
}

function renderEvents(app) {
    return `<div class="container mx-auto px-6 py-12 max-w-4xl">${(app.data.events || []).map(e => `<div class="flex mb-6 bg-white border rounded shadow-sm overflow-hidden"><div class="bg-[#1e3a2c] text-white p-6 w-24 flex flex-col items-center justify-center shrink-0"><span class="text-2xl font-bold">${e.day}</span><span class="text-[10px] uppercase font-bold">${e.month[app.lang]}</span></div><div class="p-6"><h4 class="font-bold text-[#1e3a2c] uppercase text-sm">${e.title[app.lang]}</h4><p class="text-xs text-gray-700 mt-2 italic">${e.desc[app.lang]}</p></div></div>`).join('')}</div>`;
}

function renderContact(app) {
    return `<div class="container mx-auto px-6 py-16 grid md:grid-cols-2 gap-12"><div><h2 class="text-3xl font-bold text-[#1e3a2c] uppercase mb-8">${app.menuLabels[app.lang].contact}</h2><p class="text-sm text-gray-700"><i class="fa-solid fa-location-dot text-[#c5a059] mr-3"></i> Urca, Rio de Janeiro - RJ</p></div><div class="h-64 border rounded shadow-lg overflow-hidden"><iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3674.562!2d-43.167!3d-22.955!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x997ff479429599%3A0x66c888371302868c!2sECEME!5e0!3m2!1spt-BR!2sbr!4v1714594245600" width="100%" height="100%" style="border:0;"></iframe></div></div>`;
}
