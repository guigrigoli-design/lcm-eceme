/**
 * MENU VIEWS - Versão 41.0
 * Restauração da visualização circular de Pesquisadores.
 */
function renderMenuModule(app) {
    ...
    // 2. COORDENAÇÃO LCM (Configuração Clássica)
    if (view === 'leadership') {
    ...
    // 3. PESQUISADORES (Restauração da Configuração Definida Anteriormente)
    ...
    // 5. CAPACITAÇÃO (NCNP - 3 Submenus)
    if (view === 'cnp') {
        const labels = { pt: { sub1: 'Coordenadores', sub2: 'Documentos', sub3: 'Alunos' }, en: { sub1: 'Advisors', sub2: 'Docs', sub3: 'Students' }, es: { sub1: 'Coordinadores', sub2: 'Docs', sub3: 'Alumnos' } };
        ...
        if (cnpSubView === 'coords') {
            const cList = [...(data.ic.coords || [])].sort((a,b) => (a.role[lang] || "").toLowerCase().includes("coord") ? -1 : 1);
...
function renderHome(app) {
    ...
    return `<div class="bg-gray-200 h-[400px] relative overflow-hidden">
        ...
        <div class="flex justify-center items-center space-x-10 pt-8 border-t">
            <a href="#" class="text-2xl text-[#1e3a2c] hover:text-[#c5a059]"><i class="fa-brands fa-instagram"></i></a>
            <a href="#" class="text-2xl text-[#1e3a2c] hover:text-[#c5a059]"><i class="fa-brands fa-linkedin"></i></a>
            <a href="#" class="text-2xl text-[#1e3a2c] hover:text-[#c5a059]"><i class="fa-brands fa-youtube"></i></a>
