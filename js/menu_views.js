...
function renderMenuModule(app) {
    ...
    if (view === 'cnp') {
        ...
        if (cnpSubView === 'coords') {
            const list = [...(data.ic.coords || [])].sort((a,b) => (a.role[lang] || "").toLowerCase().includes("coord") ? -1 : 1);
...
function renderDomains(app) {
    const domains = [
        { id: 1, title: { pt: "Domínio 1 - Ciência, Tecnologia, Inovação e Sustentação em Defesa", en: "Domain 1", es: "Dominio 1" } },
        { id: 2, title: { pt: "Domínio 2 - Preparo e Emprego do Poder Militar", en: "Domain 2", es: "Dominio 2" } },
        { id: 3, title: { pt: "Domínio 3 - Análise e Prospecção do Ambiente Estratégico", en: "Domain 3", es: "Dominio 3" } }
    ];
    return `<div class="container mx-auto px-6 py-12">${domains.map(d => `
        <div class="mb-16"><h3 class="text-lg font-bold text-[#1e3a2c] uppercase border-l-8 border-[#c5a059] pl-4 mb-8">${d.title[app.lang]}</h3>
