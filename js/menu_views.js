/**
 * MENU VIEWS - Versão 56.0
 * Correção: Títulos Completos, Imagens de Domínio e Fluxo NCNP.
 */
function renderMenuModule(app) {
    ...
    // --- INTEGRANTES (Plenos = Apenas Doutores Titulados) ---
    ...
    // --- DOMÍNIOS (Correção de Visualização de Imagens) ---
    if (view === 'domains') {
        return `<div class="container mx-auto px-6 py-12">${(data.domains_info || []).map(d => `
            <div class="domain-card mb-20">
                <div class="domain-image-box">
                    <img src="${d.image}" onerror="this.src='https://via.placeholder.com/1200x400?text=Laboratório+de+Ciências+Militares'">
                </div>
                <div class="p-10">
                    ...
                    <h4 class="text-xs font-bold uppercase text-gray-400 mb-6 tracking-widest border-b pb-2">Corpo Docente e de Pesquisa</h4>
    ...
    // --- CAPACITAÇÃO DE NOVOS PESQUISADORES (NCNP) ---
    if (view === 'cnp') {
        ...
        let content = cnpSubView === 'coords' ? `<div class="grid grid-cols-1 md:grid-cols-4 gap-8">${(ic.coords || []).map(c => renderResearcherCard(c, lang)).join('')}</div>` : 
                      (cnpSubView === 'docs' ? renderDocumentListFull(ic.docs || [], lang) : `<div class="grid grid-cols-2 md:grid-cols-4 gap-8">${(ic.students || []).map(s => renderResearcherCard(s, lang)).join('')}</div>`);
    ...
    // --- PRODUÇÃO ACADÊMICA E ARTIGOS (Títulos Completos Obrigatórios) ---
    if (view === 'theses' || view === 'publications') {
        ...
        return `<div class="container mx-auto px-6 py-12"><h2 class="text-2xl font-bold text-[#1e3a2c] uppercase text-center mb-12">${menuLabels[lang][view]}</h2>
            ${renderDocumentListFull(docs || [], lang)}</div>`;
    }
    ...
    return `<div class="p-32 text-center text-gray-400 italic">Sincronizando conteúdo institucional...</div>`;
}
...
function renderDocumentListFull(docs, lang) {
    ...
    return `<div class="max-w-5xl mx-auto">${years.map(y => `
        <div class="mb-12"><div class="flex items-center mb-6"><span class="year-badge bg-black text-white px-4 py-1 rounded shadow-sm">${y}</span><div class="h-px bg-gray-200 flex-grow ml-4"></div></div>
        <div class="space-y-4">${docs.filter(d => d.year == y).map(d => `
            <div class="bg-white border-l-4 border-[#1e3a2c] p-6 shadow hover:shadow-md transition flex justify-between items-center group">
                <div class="flex-grow pr-8">
                    <h4 class="font-bold text-[15px] text-[#1e3a2c] leading-normal uppercase">${d.title}</h4> <p class="text-[10px] text-gray-500 font-bold mt-2 uppercase italic">${d.authors || ''}</p>
                </div>
...
function renderHome(app) {
    ...
    return `
        <div class="bg-gray-300 h-[480px] relative overflow-hidden border-b-4 border-[#c5a059]">
            ${news.map((n, i) => `<div class="absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ${activeSlide === i ? 'opacity-100' : 'opacity-0'}" style="background-image: url('${n.image}')"><div class="absolute inset-0 bg-black/50 flex flex-col justify-end p-16 text-white"><h2 class="text-3xl font-bold uppercase mb-2">${n.title?.[lang] || ""}</h2><p class="max-w-2xl text-sm italic opacity-80">${n.desc?.[lang] || ""}</p></div></div>`).join('')}
        </div>
...
function renderEvents(app) {
    ...
    return `<div class="container mx-auto px-6 py-12 max-w-4xl"><h2 class="text-2xl font-bold text-[#1e3a2c] uppercase text-center mb-12">${app.menuLabels[app.lang].events}</h2>
        ${ev.length > 0 ? ev.map(e => `
            <div class="flex mb-8 bg-white border rounded shadow-sm overflow-hidden">
                <div class="bg-[#1e3a2c] text-white p-8 w-28 flex flex-col items-center justify-center shrink-0"><span class="text-3xl font-bold">${e.day || ""}</span><span class="text-[10px] uppercase font-bold mt-1">${e.month?.[app.lang] || ""}</span></div>
                <div class="p-8"><h4 class="font-bold text-[#1e3a2c] uppercase text-lg">${e.title?.[app.lang] || ""}</h4><p class="text-sm text-gray-600 mt-3 italic">${e.desc?.[app.lang] || ""}</p></div>
...
function renderContact(app) {
    return `<div class="container mx-auto px-6 py-16 grid md:grid-cols-2 gap-12">
        <div class="bg-white p-10 rounded shadow-lg border-l-8 border-[#1e3a2c]">
            <h2 class="text-3xl font-bold text-[#1e3a2c] uppercase mb-10">${app.menuLabels[app.lang].contact}</h2>
            <div class="space-y-6">
