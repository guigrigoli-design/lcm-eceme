function renderMenuModule(app) {
    ...
    // 2. COORDENAÇÃO LCM
    if (view === 'leadership') {
        return renderSimpleList(data.coordinators || [], lang, app.menuLabels[lang].leadership);
    }
    ...
    if (view === 'all_researchers') {
        return renderResearchers(app);
    }

    // 5. CAPACITAÇÃO (NCNP) - COM SUBMENUS
    if (view === 'cnp') {
        const labels = {
        ...
        const subMenuNav = `
            <div class="flex justify-center space-x-6 mb-12 border-b">
                <button @click="setCnpSubView('coords')" class="pb-2 text-[10px] uppercase font-bold tracking-widest ${cnpSubView === 'coords' ? 'tab-active' : 'text-gray-400'}" x-text="'${labels[lang].sub1}'"></button>
                <button @click="setCnpSubView('docs')" class="pb-2 text-[10px] uppercase font-bold tracking-widest ${cnpSubView === 'docs' ? 'tab-active' : 'text-gray-400'}" x-text="'${labels[lang].sub2}'"></button>
                <button @click="setCnpSubView('students')" class="pb-2 text-[10px] uppercase font-bold tracking-widest ${cnpSubView === 'students' ? 'tab-active' : 'text-gray-400'}" x-text="'${labels[lang].sub3}'"></button>
            </div>
        `;
        ...
        if (cnpSubView === 'coords') {
            // Ordenação: Coordenadora do Núcleo primeiro, depois Orientadores
    ...
    return `<div class="p-20 text-center italic text-gray-400">Página em construção (${lang})...</div>`;
}
...
function renderResearcherCard(r, lang) {
    return `
        <div class="flex flex-col items-center">
            ...
            <a href="${r.lattes}" target="_blank" class="text-[9px] font-bold border-2 border-[#1e3a2c] px-4 py-1 rounded-full hover:bg-[#1e3a2c] hover:text-white transition">LATTES CV</a>
        </div>`;
}

function renderDocumentListRaw(docs, lang) {
    if(!docs || docs.length === 0) return `<p class="text-center italic text-gray-400">Nenhum documento disponível.</p>`;
                        ...
                        ${docs.filter(d => d.year == y).map(d => `
                            <div class="bg-white border-l-4 border-[#1e3a2c] p-4 shadow-sm flex justify-between items-center hover:border-[#c5a059] transition">
                                <div>
                                    ...
                                    <p class="text-[10px] text-gray-500 uppercase font-bold">${d.type || ''}</p>
                                </div>
...
function renderDocumentList(docs, lang, title, theme) {
    return `<div class="container mx-auto px-6 py-12 max-w-5xl">
        <h2 class="text-2xl font-bold text-[#1e3a2c] uppercase text-center mb-12">${title}</h2>
        ${renderDocumentListRaw(docs, lang)}
    </div>`;
}

// Funções para Domínios, Pesquisadores, Eventos e Contato seguem o padrão Versão 37.0...
