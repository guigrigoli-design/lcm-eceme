/**
 * MENU VIEWS - LCM Portal v37.0
 * Renderização Trilíngue e Identidade Visual
 */
function renderMenuModule(app) {
    const { view, lang, data, activeSlide } = app;

    // 1. HOME - Carrossel e Mídias
    if (view === 'home') {
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                ${(data.coordinators || []).map(c => `
                    <div class="flex flex-col items-center">
                        ...
                        <a href="${c.lattes}" target="_blank" class="text-[10px] bg-[#1e3a2c] text-white px-6 py-1 rounded-full hover:bg-yellow-600 transition">LATTES CV</a>
                    </div>`).join('')}
            </div></div>`;
    }

    // 3. DOMÍNIOS (Ciência, Tecnologia, Inovação e Sustentação)
    if (view === 'domains') {
        const domains = [
...
function renderDocumentList(docs, lang, title, theme) {
    ...
    return `<div class="container mx-auto px-6 py-12 max-w-5xl">
        ...
        ${years.map(y => `
            <div class="mb-12"><div class="flex items-center mb-6"><span class="year-badge ${badgeClass}">${y}</span><div class="h-px bg-gray-200 flex-grow ml-4"></div></div>
            <div class="space-y-4">${docs.filter(d => d.year == y).map(d => `
                <div class="bg-white border-l-4 border-[#1e3a2c] p-5 shadow-sm flex justify-between items-center hover:border-[#c5a059] transition">
                    <div><h4 class="font-bold text-sm text-[#1e3a2c] leading-tight">${d.title}</h4>
                        <p class="text-[10px] text-gray-500 mt-1 uppercase font-bold">${d.authors || ''}</p></div>
