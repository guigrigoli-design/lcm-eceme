/**
 * MENU VIEWS - Versão 51.0 (Auditada contra Undefined)
 */
function renderMenuModule(app) {
    ...
    if (view === 'home') {
        ...
        const carousel = news.length > 0 ? `
            <div class="bg-gray-200 h-[420px] relative overflow-hidden shadow-inner border-b-4 border-[#c5a059]">
                ${news.map((n, i) => `
                    <div class="absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ${app.activeSlide === i ? 'opacity-100' : 'opacity-0'}" 
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-12 text-white">
                            <h2 class="text-2xl font-bold uppercase mb-2 drop-shadow-md">${n.title ? n.title[lang] : ""}</h2>
                            <p class="max-w-2xl text-sm italic opacity-90 drop-shadow-sm">${n.desc ? n.desc[lang] : ""}</p>
                        </div>
                    </div>`).join('')}
            </div>` : `<div class="h-[100px] bg-gray-100"></div>`;
        ...
        return `${carousel}
            <div class="container mx-auto px-6 py-16 max-w-4xl text-center">
                ...
                <div class="text-gray-700 text-base leading-relaxed italic mb-12">${intro}</div>
                <div class="flex justify-center items-center space-x-12 pt-8 border-t border-gray-100">
                    ...
                    <a href="https://youtube.com/@Laborat%C3%B3riodeCi%C3%AAnciasMilitares" target="_blank" class="text-3xl text-[#1e3a2c] hover:text-[#c5a059] transition"><i class="fa-brands fa-youtube"></i></a>
                    <a href="http://dgp.cnpq.br/dgp/espelhogrupo/1550449480693956" target="_blank" class="hover:scale-110 transition grayscale hover:grayscale-0"><img src="https://espelhogrupo.cnpq.br/dgp/img/logo_cnpq.png" class="h-10"></a>
                </div>
            </div>`;
    }
    ...
    if (view === 'domains') {
        ...
        return `<div class="container mx-auto px-6 py-12">${domains.map(d => `
            <div class="mb-20 bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden">
                <img src="${d.image}" class="domain-hero" onerror="this.src='https://via.placeholder.com/1200x400?text=LCM+Defesa'">
                <div class="p-10">
                    <h3 class="text-2xl font-bold text-[#1e3a2c] uppercase border-l-8 border-[#c5a059] pl-4 mb-6">${d.title[lang]}</h3>
                    <div class="text-gray-700 italic leading-relaxed mb-10 bg-gray-50 p-6 rounded-r-lg border-l-4 border-gray-200">${d.description[lang]}</div>
                    <h4 class="text-[10px] font-bold uppercase text-gray-400 mb-6 tracking-[0.2em] border-b pb-2">Pesquisadores do Domínio</h4>
    ...
    if (view === 'events') {
        ...
        if (events.length === 0) return `<div class="p-20 text-center italic text-gray-400 uppercase tracking-widest">Sem eventos programados.</div>`;
        ...
        return `<div class="container mx-auto px-6 py-16 grid md:grid-cols-2 gap-12">
            <div class="bg-white p-10 rounded shadow-lg border-l-8 border-[#1e3a2c]">
                ...
                <div class="space-y-8">
                    ...
                    <p class="flex items-center text-gray-800"><i class="fa-solid fa-location-dot text-[#c5a059] w-10 text-2xl"></i> Praça General Tibúrcio, 125, Urca, Rio de Janeiro - RJ</p>
                </div>
            ...
            <div class="h-[400px] rounded shadow-2xl overflow-hidden border-2 border-white"><iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3674.562!2d-43.167!3d-22.955!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x997ff479429599%3A0x66c888371302868c!2sECEME!5e0!3m2!1spt-BR!2sbr!4v1714594245600" width="100%" height="100%" style="border:0;"></iframe></div>
        </div>`;
    }
    ...
    return `<div class="p-32 text-center text-gray-400 uppercase tracking-widest italic">Processando conteúdo institucional...</div>`;
}
...
function renderResearcherCard(r, l) {
    ...
    return `<div class="flex flex-col items-center">
        ...
        <a href="${r.lattes}" target="_blank" class="text-[8px] font-bold border-2 border-[#1e3a2c] px-4 py-1.5 rounded-full mt-4 hover:bg-[#1e3a2c] hover:text-white transition uppercase tracking-tighter">Lattes CV</a>
    </div>`;
}
    ...
    return `<div class="max-w-4xl mx-auto">${years.map(y => `
        ...
        <div class="space-y-4">${docs.filter(d => d.year == y).map(d => `
            <div class="bg-white border-l-4 border-[#1e3a2c] p-6 shadow hover:shadow-md transition flex justify-between items-center group">
                <div><h4 class="font-bold text-sm text-[#1e3a2c] leading-snug group-hover:text-[#c5a059] transition uppercase">${d.title}</h4><p class="text-[10px] text-gray-500 font-bold mt-2 italic">${d.authors || ''}</p></div>
