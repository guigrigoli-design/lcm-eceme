    ...
    return `
        <div class="container mx-auto px-4 py-12">
            <div class="flex justify-between items-center mb-10 border-b pb-6">
                <div><h2 class="text-2xl font-bold text-[#1e3a2c] uppercase">Painel LCM</h2><p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Conectado: ${app.currentUser}</p></div>
                <button @click="logout()" class="bg-red-700 text-white px-4 py-2 rounded text-[10px] font-bold uppercase transition hover:bg-black">Sair</button>
            </div>
            ...
            <div class="flex justify-between items-center mb-8">
                <h3 class="text-lg font-bold text-[#1e3a2c] uppercase">${app.researcherSubView === 'andamento' ? 'Pesquisas em Curso' : 'Planejamento'}</h3>
                <button @click="showProjectForm = true" class="bg-[#c5a059] text-[#1e3a2c] px-6 py-2 rounded font-bold text-[10px] uppercase shadow hover:scale-105 transition">Publicar Pesquisa</button>
            </div>
                        ...
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <div class="flex space-x-2">
                                ${p.author === app.currentUser ? `
                                    <button @click="deleteProject(${p.id})" class="text-red-700 hover:text-red-900 transition"><i class="fa-solid fa-trash"></i></button>
                                    ${p.status === 'futuro' ? `<button @click="migrateToCurrent(${p.id})" class="text-blue-700 hover:text-blue-900 transition"><i class="fa-solid fa-rocket"></i></button>` : ''}
                                ` : ''}
                                <span class="text-[9px] font-bold bg-[#1e3a2c] text-white px-3 py-1 rounded">Domínio ${p.domainId}</span>
                            </div>
                        </div>
                        <div class="mb-4"><a href="${p.link}" target="_blank" class="text-blue-800 text-xs font-bold underline italic"><i class="fa-solid fa-link mr-2"></i>Link da Pesquisa</a></div>
                        <p class="text-sm text-gray-700 italic border-t border-black/5 pt-4 mb-4">${p.description}</p>
                        
                        <div class="bg-white/60 p-5 rounded-lg border border-black/5 mt-6">
                            <h5 class="text-[10px] font-bold uppercase text-[#1e3a2c] mb-3">Colaboradores Interessados:</h5>
                            <div class="space-y-4">
                                ${p.manifests && p.manifests.length > 0 ? p.manifests.map(m => `
                                    <div class="text-[11px] border-b border-black/5 pb-3">
                                        <div class="flex justify-between items-center mb-1">
                                            <span class="font-bold text-[#1e3a2c]">${m.author}</span>
                                        ...
                                        <p class="text-gray-600 leading-snug">"${m.text}"</p>
                                    </div>
                                `).join('') : '<p class="text-[10px] text-gray-400 italic">Nenhuma manifestação registrada.</p>'}
                            </div>
                        </div>

                        <div class="text-right mt-6">
                            <button @click="selectedProject = ${p.id}; showInterestModal = true" class="bg-[#1e3a2c] text-white px-8 py-2 rounded text-[10px] font-bold uppercase hover:bg-black transition">Manifestar Interesse</button>
                        </div>
                    </div>`).join('')}
            </div>
            ...
            <div x-show="showProjectForm" class="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4" x-cloak>
                <div class="bg-white rounded-lg w-full max-w-2xl p-8 relative overflow-y-auto max-h-[90vh]">
                    ...
                    <h3 class="text-xl font-bold text-eb-green uppercase border-b pb-4 mb-6">Nova Pesquisa</h3>
                    <div class="space-y-4">
                        ...
                        <textarea x-model="newProj.description" placeholder="Resumo e CRediT necessários" class="w-full border p-2 rounded text-sm h-32"></textarea>
                        
                        <div>
                            <label class="block text-[10px] font-bold uppercase text-gray-500 mb-2 italic">Selecione o CRediT desejado:</label>
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                                <template x-for="opt in creditOptions" :key="opt">
                                    <label class="flex items-center space-x-2 text-[9px] bg-gray-50 p-2 rounded cursor-pointer">
            ...
            <div x-show="showInterestModal" class="fixed inset-0 z-[110] bg-black/90 flex items-center justify-center p-4" x-cloak>
                <div class="bg-white rounded-lg w-full max-w-2xl p-8 relative overflow-y-auto max-h-[90vh]">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] font-bold uppercase text-gray-500 mb-2">Selecione as áreas CRediT de interesse:</label>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 bg-gray-50 p-4 rounded border">
                                <template x-for="role in creditOptions" :key="role">
                                    <label class="flex items-center space-x-2 text-[10px] cursor-pointer hover:text-eb-gold transition">
                        ...
                        <textarea x-model="manifestForm.text" class="w-full border p-4 rounded text-sm h-32 outline-none" placeholder="Detalhes do seu interesse..."></textarea>
                        <button @click="submitManifest()" class="w-full bg-[#c5a059] text-[#1e3a2c] font-bold py-4 rounded text-xs uppercase hover:bg-yellow-600 transition shadow-lg">Registrar Manifestação</button>
                    </div>
                </div>
            </div>
        </div>`;
...
function renderResearcherLogin(app) {
    return `<div class="py-20 flex justify-center"><div class="bg-white p-10 rounded shadow-2xl border-t-8 border-[#c5a059] w-full max-w-md text-center">
        <h3 class="font-bold uppercase text-[#1e3a2c] mb-6">Acesso Restrito</h3>
        <div class="space-y-4">
