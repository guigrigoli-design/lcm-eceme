/**
 * RESEARCHER MODULE - Versão 43.0
 * Gestão Temporal e Colaboração CRediT
 */
    ...
    return `
        <div class="container mx-auto px-4 py-12">
            <div class="flex justify-between items-center mb-10 border-b pb-6">
                <div><h2 class="text-2xl font-bold text-[#1e3a2c] uppercase">Painel de Colaboração</h2><p class="text-[10px] text-gray-500 font-bold uppercase mt-1">Sessão: ${app.currentUser}</p></div>
            ...
            <div x-show="showProjectForm" class="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4" x-cloak>
                <div class="bg-white rounded-lg w-full max-w-2xl p-8 relative">
                    ...
                    <h3 class="text-xl font-bold text-eb-green uppercase border-b pb-4 mb-6">Publicação Estratégica</h3>
                    <div class="space-y-4">
                        <input type="text" x-model="newProj.title" placeholder="Título" class="w-full border p-2 rounded text-sm">
                        <input type="url" x-model="newProj.link" placeholder="Link (URL)" class="w-full border p-2 rounded text-sm">
                        ...
                        <textarea x-model="newProj.description" placeholder="Descrição" class="w-full border p-2 rounded text-sm h-32"></textarea>
            ...
            <div x-show="showInterestModal" class="fixed inset-0 z-[110] bg-black/90 flex items-center justify-center p-4" x-cloak>
                <div class="bg-white rounded-lg w-full max-w-lg p-8 relative">
                    ...
                    <h3 class="text-xl font-bold text-eb-green uppercase border-b pb-4 mb-6">Manifestar Interesse (CRediT)</h3>
                    <div class="space-y-4">
                        <textarea x-model="manifestForm.text" class="w-full border p-4 rounded text-sm h-32" placeholder="Sua proposta de colaboração..."></textarea>
...
function renderResearcherLogin(app) {
    return `<div class="py-20 flex justify-center"><div class="bg-white p-10 rounded shadow-2xl border-t-8 border-[#c5a059] w-full max-w-md text-center">
        <h3 class="font-bold uppercase text-[#1e3a2c] mb-6">Login - Área do Pesquisador</h3>
        <div class="space-y-4">
